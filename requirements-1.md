...
### 9.2 Datastore: RocksDB

- **RocksDB** is the chosen embedded datastore for all backend persistence.
- All domain entities (teams, projects, specs, tasks, agents, commits, events, settings) are stored in RocksDB.
- Use RocksDB column families to logically partition data domains (e.g., separate column families for auth, projects, events, agent state).
- Leverage RocksDB's sorted key-value model for efficient range queries over time-ordered data (event logs, commit histories, audit trails).
- The embedded nature of RocksDB eliminates external database dependencies, simplifying deployment and operational overhead.
- Backup and snapshot capabilities should use RocksDB's built-in checkpoint/backup mechanisms.
- Key encoding schemes must support the context hierarchy (team/project/spec/task) for efficient prefix-based lookups.
...
10. RocksDB-backed persistence for all domain data.
...
16. All persistent data is stored in and retrievable from RocksDB.
