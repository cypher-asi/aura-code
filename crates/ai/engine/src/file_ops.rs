...
/// bodies. Keeps: pub struct/enum definitions (with fields/variants),
/// pub trait signatures, pub fn signatures, and impl block method signatures.
/// Typically produces ~25-30% of the original file size while preserving
...
/// Returns a compact representation with struct/enum definitions, trait
/// signatures, and fn signatures -- bodies are replaced with `{ ... }`.
pub fn extract_signatures_from_content(content: &str) -> String {
    ...
    while i < lines.len() {
        ...
        // Pass through `use`, `mod`, and attribute lines at the top level
        ...
        // Skip plain comments and blank lines
        if trimmed.starts_with("//") || trimmed.is_empty() {
            i += 1;
            continue;
        }

        // Skip attributes but remember them for the next item
        if trimmed.starts_with("#[") {
            output.push_str(trimmed);
            output.push('\n');
            i += 1;
        ...
        // pub struct / pub enum -- keep full definition including fields/variants
        ...
        // pub trait -- keep method signatures, drop default impl bodies
        ...
        // impl blocks -- keep method signatures, drop bodies
        ...
        // pub fn / pub async fn at top level -- signature only
        ...
        // pub type aliases and pub const -- keep as-is
...
/// Extract a full braced block (struct/enum) preserving all content.
...
/// Extract trait block keeping only method signatures (dropping default bodies).
...
/// Extract impl block keeping only method signatures, dropping bodies.
    ...
    for j in start..lines.len() {
        ...
        if started && depth > 1 {
            ...
            if is_fn && trimmed.contains('{') {
                ...
                // fn signature spans multiple lines; just output the sig line
                result.push_str("    ");
                result.push_str(trimmed);
                result.push('\n');
                continue;
        ...
        // Skip non-fn non-signature lines inside the impl body (e.g. let bindings)
...
/// Extract just the function signature (up to but not including `{`).
...
/// Skip past a braced block starting at the given line, returning the index
