...
fn test_detect_blocked_reads_blocks_third() {
    ...
    assert_eq!(
        blocked,
        vec![0],
        "3rd full read of same file should be blocked"
    );
...
fn test_detect_blocked_reads_different_files_independent() {
    ...
    assert!(
        blocked_b.is_empty(),
        "2nd read of b.rs should not be blocked"
    );
}
...
fn test_detect_blocked_writes_blocks_second_attempt_same_file() {
    ...
    assert_eq!(
        second,
        vec![0],
        "second consecutive write should be blocked"
    );
}
...
fn test_detect_write_file_cooldowns_blocks_write_only() {
    ...
    assert_eq!(
        blocked,
        vec![0],
        "cooldown should block write_file but not edit_file"
    );
}
...
fn test_detect_blocked_write_failures_blocks_at_three() {
    ...
    assert_eq!(
        blocked,
        vec![0, 1],
        "3 failures should block both write and edit"
    );
}
...
fn test_detect_blocked_write_failures_independent_per_file() {
    ...
    assert_eq!(
        blocked,
        vec![0],
        "only a.rs (3 failures) should be blocked, not b.rs (1 failure)"
    );
}
...
fn test_detect_blocked_exploration_not_blocked() {
    let calls = vec![
        ...
        ToolCall {
            ...
            input: serde_json::json!({"query": "fn main"}),
        },
        ToolCall {
            id: "t3".into(),
            name: "write_file".into(),
...
async fn test_exploration_hard_block_at_limit() {
    ...
    assert!(
        blocked_count > 0,
        "should have blocked at least one exploration call after limit"
    );
}
...
async fn test_exploration_unblocks_after_write() {
    let responses = vec![
        MockResponse::tool_use(vec![
            ToolCall {
                id: "r1".into(),
                name: "read_file".into(),
                input: serde_json::json!({"path": "f1.rs"}),
            },
            ToolCall {
                id: "r2".into(),
                name: "read_file".into(),
                input: serde_json::json!({"path": "f2.rs"}),
            },
            ToolCall {
                id: "r3".into(),
                name: "read_file".into(),
                input: serde_json::json!({"path": "f3.rs"}),
            },
            ToolCall {
                id: "r4".into(),
                name: "read_file".into(),
                input: serde_json::json!({"path": "f4.rs"}),
            },
        ])
        .with_tokens(50, 30),
        MockResponse::tool_use(vec![
            ToolCall {
                id: "r5".into(),
                name: "read_file".into(),
                input: serde_json::json!({"path": "f5.rs"}),
            },
            ToolCall {
                id: "r6".into(),
                name: "read_file".into(),
                input: serde_json::json!({"path": "f6.rs"}),
            },
            ToolCall {
                id: "r7".into(),
                name: "read_file".into(),
                input: serde_json::json!({"path": "f7.rs"}),
            },
            ToolCall {
                id: "r8".into(),
                name: "read_file".into(),
                input: serde_json::json!({"path": "f8.rs"}),
            },
        ])
        .with_tokens(50, 30),
        MockResponse::tool_use(vec![
            ToolCall {
                id: "r9".into(),
                name: "read_file".into(),
                input: serde_json::json!({"path": "f9.rs"}),
            },
            ToolCall {
                id: "r10".into(),
                name: "read_file".into(),
                input: serde_json::json!({"path": "f10.rs"}),
            },
            ToolCall {
                id: "r11".into(),
                name: "read_file".into(),
                input: serde_json::json!({"path": "f11.rs"}),
            },
            ToolCall {
                id: "r12".into(),
                name: "read_file".into(),
                input: serde_json::json!({"path": "f12.rs"}),
            },
        ])
        .with_tokens(50, 30),
        MockResponse::tool_use(vec![ToolCall {
            id: "w1".into(),
            name: "write_file".into(),
            input: serde_json::json!({"path": "out.rs", "content": "done"}),
        }])
        .with_tokens(50, 30),
        MockResponse::tool_use(vec![ToolCall {
            id: "r13".into(),
            name: "read_file".into(),
            input: serde_json::json!({"path": "f13.rs"}),
        }])
        .with_tokens(50, 30),
    ...
    assert!(
        post_write_read_succeeded,
        "read after write should succeed (exploration unblocked)"
    );
}
