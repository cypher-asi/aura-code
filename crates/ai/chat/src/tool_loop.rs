...
fn smart_compact(tool_name: &str, content: &str) -> String {
    ...
    if tool_name == "read_file" && looks_like_rust(content) {
...
fn looks_like_rust(content: &str) -> bool {
    ...
    if start.contains("use ") || start.contains("pub use ") { rust_signals += 1; }
    if start.contains("pub fn ") || start.contains("fn ") { rust_signals += 1; }
    if start.contains("pub struct ") || start.contains("struct ") { rust_signals += 1; }
    if start.contains("impl ") { rust_signals += 1; }
    if start.contains("pub mod ") || start.contains("mod ") { rust_signals += 1; }
    if start.contains("pub enum ") || start.contains("enum ") { rust_signals += 1; }
    if start.contains("pub trait ") || start.contains("trait ") { rust_signals += 1; }
    if start.contains("-> Result<") || start.contains("-> Option<") { rust_signals += 1; }

    rust_signals >= 3
}

/// Lightweight Rust signature extraction for compaction.  Keeps: use/mod
/// declarations, pub struct/enum definitions (with fields), pub fn/trait/impl
/// signatures (without bodies).  Produces a compact API surface view.
fn extract_rust_signatures(content: &str) -> String {
    ...
    while i < lines.len() {
        ...
        if (trimmed.starts_with("pub struct ") || trimmed.starts_with("pub enum "))
            && !trimmed.ends_with(';')
        {
            let end = skip_braced_block_compact(&lines, i);
            for j in i..=end.min(lines.len().saturating_sub(1)) {
                output.push_str(lines[j].trim());
                output.push('\n');
            }
            i = end + 1;
            continue;
        ...
        if trimmed.starts_with("pub trait ") {
            let end = emit_block_signatures(&lines, i, &mut output);
            i = end + 1;
            continue;
        }

        if trimmed.starts_with("impl ") || trimmed.starts_with("impl<") {
            let has_pub = has_pub_methods(&lines, i);
            let is_trait_impl = trimmed.contains(" for ");
            if has_pub || is_trait_impl {
                let end = emit_block_signatures(&lines, i, &mut output);
                i = end + 1;
            } else {
                i = skip_braced_block_compact(&lines, i) + 1;
            }
            continue;
        }
        ...
        {
            let sig = extract_fn_sig(&lines, i);
            output.push_str(&sig);
            output.push_str(" { ... }\n");
            i = skip_braced_block_compact(&lines, i) + 1;
            continue;
...
fn skip_braced_block_compact(lines: &[&str], start: usize) -> usize {
    let mut depth: i32 = 0;
...
fn emit_block_signatures(lines: &[&str], start: usize, output: &mut String) -> usize {
    let mut depth: i32 = 0;
    let mut started = false;
...
fn has_pub_methods(lines: &[&str], start: usize) -> bool {
    let mut depth: i32 = 0;
    let mut started = false;
    for j in start..lines.len() {
        let trimmed = lines[j].trim();
        ...
        if started && depth > 1
            && (trimmed.starts_with("pub fn ") || trimmed.starts_with("pub async fn ")
                || trimmed.starts_with("pub const fn ") || trimmed.starts_with("pub(crate) fn ")
                || trimmed.starts_with("pub(crate) async fn "))
        {
            return true;
        }
...
fn extract_fn_sig(lines: &[&str], start: usize) -> String {
