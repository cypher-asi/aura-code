...
async fn test_iteration_token_usage_events_match_mock_responses() {
    ...
    // IterationTokenUsage reports cumulative totals
...
async fn test_credit_budget_stops_loop() {
    // Each iteration uses ~estimate_credits(opus, 1000, 500) credits.
    // We set a budget so small that after 1-2 iterations the loop should stop.
    ...
    // Set a budget that allows ~1 iteration: estimate_credits(opus, 100k, 50k)
    // is around (0.5*100k*5 + 0.5*100k*5*0.1 + 50k*25)/1M * 114286 ≈ 1714 credits
    // A budget of 2000 should stop after 1 iteration.
...
async fn test_credit_budget_warnings_emitted() {
    // We need enough iterations that cumulative credits pass 30% and 60% of budget.
    // With tokens(10_000, 5_000): estimate ≈ ~171 credits per iteration.
    // Budget = 1000 → 30% = 300 (after ~2 iterations), 60% = 600 (after ~4).
    let responses: Vec<MockResponse> = (0..10).map(|i| {
        MockResponse::tool_use(vec![ToolCall {
            id: format!("t{i}"),
            name: "do_thing".into(),
    ...
    // The loop should have stopped at some point due to budget
...
async fn test_insufficient_credits_mid_loop() {
    ...
    // Start with a large balance so iteration 1 passes, but drain it
    // after the first debit so iteration 2 fails.
    // 10_000 tokens opus → debit ≈ round((10K*5 + 5K*25)/1M * 114286) ≈ ~20 credits
    // Balance of 100 is enough for 1 iteration but not 2 (the balance
    // endpoint will return the post-debit balance which is < estimated).
