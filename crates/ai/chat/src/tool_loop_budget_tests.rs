...
async fn test_token_accumulation_across_many_iterations() {
    let token_pairs: Vec<(u64, u64)> =
        vec![(100, 50), (200, 80), (150, 60), (300, 120), (250, 100)];
    let mut responses: Vec<MockResponse> = token_pairs
        .iter()
        .enumerate()
        .map(|(i, (inp, out))| {
...
async fn test_iteration_token_usage_events_match_mock_responses() {
    ...
    while let Ok(evt) = event_rx.try_recv() {
        if let ToolLoopEvent::IterationTokenUsage {
            input_tokens,
            output_tokens,
        } = evt
        {
    ...
    for (i, (inp, out)) in token_pairs.iter().enumerate() {
        ...
        assert_eq!(
            usage_events[i].0, cumulative_input,
            "iteration {i} cumulative input mismatch"
        );
        assert_eq!(
            usage_events[i].1, cumulative_output,
            "iteration {i} cumulative output mismatch"
        );
    }
}
...
async fn test_credit_budget_stops_loop() {
    ...
    assert!(
        result.iterations_run < 10,
        "should have stopped early due to credit budget"
    );
    assert!(
        result.insufficient_credits,
        "credit budget exceeded should set insufficient_credits"
    );
...
async fn test_insufficient_credits_mid_loop() {
    use aura_billing::testutil::{make_test_llm_stateful, MockBillingState};
    ...
    assert!(
        result.insufficient_credits,
        "should flag insufficient_credits"
    );
    assert!(
        result.iterations_run >= 1,
        "at least 1 iteration should have run"
    );
}
