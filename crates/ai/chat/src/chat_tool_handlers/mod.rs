...
impl ChatToolExecutor {
    ...
    pub(crate) fn require_storage(&self) -> Result<&std::sync::Arc<aura_storage::StorageClient>, ToolExecResult> {
        self.storage_client
            .as_ref()
    ...
    pub(crate) fn storage_and_jwt(&self) -> Result<(&std::sync::Arc<aura_storage::StorageClient>, String), ToolExecResult> {
        let storage = self.require_storage()?;
        let jwt = self.get_jwt()?;
        Ok((storage, jwt))
    }

    pub(crate) async fn resolve_project_path(&self, project_id: &ProjectId, rel: &str) -> Result<std::path::PathBuf, ToolExecResult> {
