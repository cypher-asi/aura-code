...
fn test_build_tool_result_blocks_duplicate_read_returns_stop_message() {
    ...
    let results2 = vec![ToolCallResult {
        tool_use_id: "t2".into(),
        content: "fn main() {}".into(),
        is_error: false,
        stop_loop: false,
    ...
    assert!(
        content.contains("STOP: File already read"),
        "duplicate read should return STOP message"
    );
}
...
fn test_build_tool_result_blocks_write_invalidates_cache() {
    ...
    let write_calls = vec![ToolCall {
        id: "w1".into(),
        name: "write_file".into(),
        input: serde_json::json!({"path": "a.rs", "content": "new content"}),
    }];
    let write_results = vec![ToolCallResult {
        tool_use_id: "w1".into(),
        content: "ok".into(),
        is_error: false,
        stop_loop: false,
    ...
    assert!(
        !cache.contains_key("a.rs"),
        "write_file should invalidate read cache"
    );
}
...
fn test_build_tool_result_blocks_edit_invalidates_cache() {
    ...
    let edit_calls = vec![ToolCall {
        id: "e1".into(),
        name: "edit_file".into(),
        input: serde_json::json!({"path": "a.rs"}),
    }];
    let edit_results = vec![ToolCallResult {
        tool_use_id: "e1".into(),
        content: "ok".into(),
        is_error: false,
        stop_loop: false,
    ...
    assert!(
        !cache.contains_key("a.rs"),
        "edit_file should invalidate read cache"
    );
}
...
fn test_summarize_edit_file_input_long_content_truncated() {
    ...
    assert!(
        old.contains("chars omitted"),
        "long old_text should be truncated"
    );
