...
async fn list_directory(Json(req): Json<ListDirectoryRequest>) -> Json<serde_json::Value> {
    let target = std::path::Path::new(&req.path);
    if !target.exists() {
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async move {
            ...
            let app = aura_server::create_router_with_frontend(app_state, frontend_dir)
                ...
                .route("/api/list-directory", axum_post(list_directory))
                .route(
                    "/api/open-ide",
